...
// Editor structure
class Editor {
    public char rows[1024][1024]; // text rows
    public int rowCount;
    ...
    public char command_buffer[256];
    public char status_msg[256];
...
// Terminal control
public void disableRawMode(TerminalHandler th) {
    th.disableRawMode();
}
...
// Open file
public void openFile(char fname[],Editor E) {
...
public void processKeypress() {
    ...
    if (E.insert_mode==0) {
        ...
        else if (c == ':') E.command_mode = 1;
        else moveCursor(c);
    } 
    else {
        if (c == 27) E.insert_mode = 0;
        else if (c == 127 || c == '\b') { 
            if (E.cx > 0) {
